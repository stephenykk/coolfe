...
在 Taro 中尺寸单位建议使用 px、 百分比 %, 设计稿量出来多少就写多少 px, 小程序会转为 rpx， h5 会转为 rem
...
js 生成的样式，用 pxTransform 转换为合适的单位. 如 `Taro.pxTransform(10)`
...
可以使用 JavaScript 表达式；if 和 for 语句不是表达式，不能用在属性上
...
_PropTypes 目前在小程序端还有些问题，但在 H5 端可以使用_
...
_taro 中 map 返回的结果为模板字符串，不应再进一步处理_
...
- _继承样式，如 font 、 color ，会从组件外（父组件）继承到组件内。但是引用组件时在组件节点上书写的 className 无效。 （具体解决方案请参见下面的外部和全局样式介绍。）_
