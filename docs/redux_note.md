redux notes
===
转自[阮一峰的redux教程](http://www.ruanyifeng.com/blog/2016/09/redux_tutorial_part_one_basic_usages.html),说的很清楚
...
createStore函数接受另reducer函数作为参数，返回新生成的 Store 对象。
...
### Action
State 的变化，会导致 View 的变化。但是，用户接触不到 State，只能接触到 View。Action 就是 用户与View交互 所发出的通知，表示 State 应该要发生变化了。
...
store.dispatch方法会触发 Reducer 的自动执行。为此，Store 需要知道 Reducer 函数，做法就是在生成 Store 的时候，将 Reducer 传入createStore方法 `const store = createStore(reducer)`。
