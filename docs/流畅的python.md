...
交互式控制台 REPL 是一个很好地学习工具

## 数据模型

数据模型其实是对 Python 框架的描述，规范了这门语言构建模块的接口，如: 序列、迭代器、上下文管理、函数和类

**Python 语言的一致性**

让 Python 的使用者和 Python 的发明者拥有一样的工具和能力

**元对象协议**  
MetaObjectProtocol MOP 元对象是指那些对构建语言本身很重要的对象，_也可理解为构建语言核心的 API_

Python 和 Ruby 提供丰富的元对象协议

**魔术方法**  
前后带双下划线的方法，如 `__init__`，魔术方法大多数都是由 Python 解释器自动调用的。

很多内置的全局方法调用以及运算符，其实都是调用对象的特殊方法，如 `abs` , `str` , `*`

```Python
# __getitem__()
aDict = {'name': 'alice', 'like': 'dreaming'}
# 底层隐式调用 aDict.__getitem__('name')
aDict['name']
...
Python 的序列类型包括：字符串、字节序列、列表、数组、元组

序列类型常用操作: 迭代 切片 排序 拼接

**容器序列**

可以存放不同类型, 容器类型的变量保存对象的引用
...
**扁平序列**  
只能存放一种类型，扁平类型的变量是在一段连续的内存空间保存对象的值
...
**字符串格式化**  
`%` 占位符 和 `strPattern.format()` 两种方式

```Python
print('%s is good' % 'apple')
'{} is good'.format('apple')
...
`__repr__` 表示如何用代码构建出被打印的对象, `__str__` 表示被打印对象的可读字符串表示。
...
colors = ['black', 'white']
sizes = ['S', 'M', 'L']
allShirts = [ (color, size) for color in colors for size in sizes ]
```
...
```Python
characters = 'abcd'
for val in (c for c in characters):
    print(val)
```
...
```python
tuple(ord(c) for c in characters)

import array
array.array('I', (ord(c) for c in characters))
...
# 实际调用 nums.__getitem__(slice(5,1,-1))
nums[5:1:-1]
...
memoryview.cast 会把同一块内存中的内容打包成一个 memoryview 对象
...
dict 是 Python 语言的基石 _模块命名空间_ ,_实例属性_ , _关键字参数_
...
原子数据类型都是可散列的(_hashable_): int float str bool bytes

frozenset 也是可散列的, 元组不一定是可散列的
...
集合的作用是去重，很多对象的聚合，对象必须是 hashable

set 不是 hashable  
fronzenset 是 hashable
...
-   hash 是一个稀疏数组(小于 1/3 空闲 则扩容)
-   表元(键值对对应一个表元，大小相同 可通过偏移访问)
-   对键求 hash 值，用 hash 值的最低几位作为 hash 索引, 查找表元
-   没找到表元，抛出 KeyError; 找到表元，则看表元的键是否等于查找的键，相等则返回表元的键值；若不相等，则发生散列冲突；对键的 hash 值，取更多的低位数重新查找
-   根据 hash 的拥挤程度进行扩容，hash 增大，hash 值所占位数和用作索引的位数也随之增大，以减少散列冲突的发生
-   所有用户自定义对象默认可散列，hash(yourObj) === id(yourObj)
-   hash 是稀疏数组，比较占空间，元组和具名元组会比较节约空间
-   键查询非常快，dict 是典型的空间换时间，无视数据量大小的快速访问
-   通常地，键的顺序和添加的顺序一致
...
python3 中，str 对象中获取到的元素是 unicode 字符
python2 中，str 对象中获取到的元素是字节序列

码位: 字符在编码表中对应的数值 (_编码表 = 字符编码_)
...
> bytes 和 bytearray 对象的每个元素都是介于 0-255 之间的整数
...
str 对象的方法，支持 bytes 和 bytearray 类型的有:
...
memoryview 允许在二进制数据结构之间共享内存

struct 模块，从二进制序列中提取结构化数据
...
unicodedata.normalize 函数可以将 unicode 规范化
...
unicode 数据库，保存了码位和字符名称的映射关系，还有字符的元数据，如：isidentifier, isprintable, isdecimal, isnumeric 等方法都是依赖 unicode 数据实现的

unicodedata 模块中用于获取元数据的方法:

-   unicodedata.name()
-   unicodedata.numeric()

双模式 API,即函数可接受字符串或字节序列作为参数
...
map 和 filter 是内置函数，同样的效果用 列表推导或生成器表达式更方便
...
> python3 中 map 和 filter 返回生成器, python2 中 map 和 filter 返回列表

> python3 reduce 在 functools 模块中; python2 reduce 是内置函数
...
-   自定义函数 (使用 def 语句或 lambda 表达式创建)
-   内置函数 (len, print, list)
-   内置方法 (str.split)
-   自定义方法 (自定义类中的方法)
-   类 (调用类时执行 **new** 和 **init**)
-   类的实例 (若类定义了 **call** 方法，则实例也是可调用的)
-   生成器函数 (使用 yield 关键字)
...
Python 中常用的高阶函数: map filter functools.reduce all any functools.partial mix max sorted

> lambda map filter reduce 最早出现在 Lisp 中，最早的一门函数式编程语言
> Haskell 同样是一门函数式编程语言，python 从中借鉴了列表推导
> Python 不支持尾递归优化
...
> 策略模式，定义一系列算法并封装起来，且它们可以互相替代，(_电商场景，不同的促销策略 或者说 折扣优惠策略_)
...
functools 模块的装饰器有: lru_cache 和 singledispatch functools.wraps
...
闭包是延申了作用域的函数，能访问父函数定义的变量(_自由变量_)
...
a.**eq**(b)。 继承自 object 的 **eq** 方法比较两个对象的 ID， 结
果与 is 一样。 但是多数内置类型使用更有意义的方式覆盖了 **eq**
方法， 会考虑对象属性的值。
...
毁： CPython 会在对象上调用 **del** 方法（如果定义了） ， 然后释放
分配给对象的内存。
